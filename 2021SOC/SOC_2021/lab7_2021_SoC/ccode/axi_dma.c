...
void AXI_DMA_IP_TEST(void)
{
	...
	Xil_DCacheFlushRange((UINTPTR)(&Source_RX), TEST_SIZE*8/2);

	//Start DDR to IP transfer
	AXI_DMA_Transfer((UINTPTR)(&Source_TX)	,TEST_SIZE*8,	XAXIDMA_DMA_TO_DEVICE);
	/*
	 * Wait TX done
	 */
	...
	AXI_DMA_Transfer((UINTPTR)(&Source_RX),TEST_SIZE*8/2,	XAXIDMA_DEVICE_TO_DMA);
	/*
	 * Wait RX done
	 */
